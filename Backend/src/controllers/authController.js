...
const { generateOTP } = require('../models/GenerateOTP.js');
...
// Forgot Password send email OTP and verification and update new password
...
exports.verifyOTP = async(req,res) => {
    try{
        ...
        // Check expiry FIRST so expired OTPs give the right message
        ...
        // Compare as strings — generateOTP() returns a Number, frontend sends a String
...
exports.resetPassword = async (req, res) => {
    try {
        ...
        // Need .select('+password') so Mongoose sees the field as modified
        const user = await User.findOne({ email }).select('+password');
        if (!user) {
            return res.status(404).json({
                success: false,
        ...
        // Assign plain text — the pre-save hook will hash it automatically
